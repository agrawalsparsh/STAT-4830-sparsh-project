...
from gen_simulations import gen_simulation, sim_mock_game, play_human, gen_fully_batched_simulations
...
def iteration(iter, num_sims=SIM_CONFIG["sims_per_iter"]):
    ...
    # Use fully batched simulation approach instead of sequential generation
   # print(f"Generating {num_sims} simulations with fully batched approach...")
    regret_samples, policy_samples = gen_fully_batched_simulations(
        policy_network, 
        regret_network, 
        n_actions, 
        SIM_CONFIG["random_games"], 
        SIM_CONFIG["prob_random"],
        num_envs=num_sims
    )
    
    print(f"Generated {len(regret_samples)} regret samples and {len(policy_samples)} policy samples")
...
def train_and_play_human():
    # Check for policy network weights directory
    weights_dir = "policy_network_weights"
    weights_path = os.path.join(weights_dir, "policy_network.pth")
    
    policy_network = PolicyNetwork().to(device)
    
    # If weights directory doesn't exist, create it
    ...
    if os.path.exists(weights_path):
        ...
        policy_network.load_state_dict(torch.load(weights_path))
    else:
        print("No existing weights found. Training new model...")
        # Check for reservoir data and train
        if not os.path.exists(POLICY_RESERVOIR_PATH):
        ...
        torch.save(policy_network.state_dict(), weights_path)
