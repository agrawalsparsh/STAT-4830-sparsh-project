from reservoir_sampling import RegretReservoir, PolicyReservoir
...
def iteration(iter, num_sims=SIM_CONFIG["sims_per_iter"]):
    ...
    if not os.path.exists(POLICY_RESERVOIR_PATH):
        policy_reservoir = PolicyReservoir(size=TRAIN_CONFIG["policy_reservoir_size"])
        regret_reservoir = RegretReservoir(size=TRAIN_CONFIG["regret_reservoir_size"])
    else:
        with open(POLICY_RESERVOIR_PATH, "rb") as f:
            policy_reservoir = pickle.load(f)
            #policy_reservoir.size = TRAIN_CONFIG["policy_reservoir_size"]
            #policy_reservoir.data = policy_reservoir.get_data()[-1*TRAIN_CONFIG["policy_reservoir_size"]:]
            #from types import MethodType
            #policy_reservoir.get_replacement_probability = MethodType(lambda self: 1, policy_reservoir)


        with open(REGRET_RESERVOIR_PATH, "rb") as f:
            regret_reservoir = pickle.load(f)
            #regret_reservoir.size = TRAIN_CONFIG["regret_reservoir_size"]
            #regret_reservoir.data = regret_reservoir.get_data()[-1*TRAIN_CONFIG["regret_reservoir_size"]:]
