...
'''
def gen_simulation(policy_network, regret_network, iter_epsilon):
    ']'' 
    Inputs:
      policy_network -> network used to predict the average strategy (trained offline)
      regret_network -> network used to predict the value/regret of taking an action in a given state: (state, action) -> scalar
      iter_epsilon -> for each action we draw a random uniform and if < iter_epsilon, choose a random action; otherwise, select according to our instantaneous strategy
    Outputs:
      regret_samples -> list of [(State, Action, Value, Reach)] samples to train the regret network
      policy_samples -> list of [(State, Probabilities, Reach)] samples to train the policy network
    ']''
    ...
    env.reset()
    #log_reach = 1  # env.player_log_prob if available
    ...
    while not env.game_over:
        ...
        for p in range(env.num_players):
            obs = env.get_observation_for_agent(p)
            if p == 0:
                # For player 0, compute the instantaneous strategy using regret matching directly on the regret network outputs.
                expected_outcomes = predict_counter_factuals(regret_network, obs)
                # Compute a baseline; for instance, using the current policy (could be uniform or from the policy network)
                cur_policy = predict_policy(policy_network, obs)
                baseline_value = np.dot(expected_outcomes, cur_policy)
                # Compute regrets relative to the baseline
                regrets = expected_outcomes - baseline_value
                pos_regrets = np.maximum(regrets, 0)
                sum_pos_regrets = np.sum(pos_regrets)
                if sum_pos_regrets > 1e-8:
                ...
                # Store this sample for training the policy network (the average strategy).
                #log_reach = 0 #set equal weight to samples
                policy_samples.append([obs, probabilities, 1])# np.exp(log_reach)])
                
                # Select an action.
                most_likely = DISCRETE_ACTION_SPACE[np.argmax(probabilities)]
                if np.random.random() < iter_epsilon:
                    action = np.random.choice(DISCRETE_ACTION_SPACE)
                else:
                    action = most_likely

                # Calculate delta reach with numerical stability.
                if np.abs(action - most_likely) < 0.01:
                    delta_reach = np.log(iter_epsilon / len(DISCRETE_ACTION_SPACE) + 1 - iter_epsilon)
                    regret_samples.append([obs, action, '', np.exp(delta_reach)]) 
                else:
                    delta_reach = np.log(iter_epsilon / len(DISCRETE_ACTION_SPACE))
                    regret_samples.append([obs, action, '', np.exp(delta_reach)]) 

                #log_reach += delta_reach
                #regret_samples.append([obs, action, '', np.exp(log_reach)])  # Realized value to be filled later.
                actions[f"agent_{p}"] = action

            else:
                # For other players, we still use the policy network (representing a fixed average strategy) for action selection.
                cur_policy = predict_policy(policy_network, obs)
                action_idx = np.random.choice(len(DISCRETE_ACTION_SPACE), p=cur_policy)
                action = DISCRETE_ACTION_SPACE[action_idx]
                actions[f"agent_{p}"] = action
                #log_reach += np.log(cur_policy[action_idx])

        env.step(actions)
    
    reward = env.get_reward()[0]
    for sample in regret_samples:
        sample[2] = reward  # Fill in the realized value for each regret sample.
